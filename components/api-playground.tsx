...
export function APIPlayground({
  ...
  const updateConfig = <K extends keyof APIConfig>(
    ...
    setConfig((prev) => {
      ...
      if (
        ...
        const queryChanged =
          currentQueryKeys.size !== newQueryKeys.size ||
        ...
        const pathChanged =
          currentPathKeys.size !== newPathKeys.size ||
  ...
  const updateActiveTabMapItem = (
    ...
    if (type === "query") {
      ...
      if (oldItem.key) {
        const escapedOldKey = oldItem.key.replace(
          /[.*+?^${}()|[\]\\]/g,
          "\\$&",
        );
  ...
  const removeActiveTabMapItem = (type: keyof typeof map, itemId: string) => {
    const itemToRemove = config[type]?.find(
      (item: Parameter) => item.id === itemId,
    );
    const filteredItems =
      config[type]?.filter((item: Parameter) => item.id !== itemId) || [];
    ...
    if (type === "query") {
      ...
      const currentUrl = config.url || "";
      const escapedKey = itemToRemove.key.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&",
      );
  ...
  const testApi = async () => {
    ...
    try {
      ...
      const status =
        (response &&
          (response.status_code ?? response.status ?? response.statusCode)) ??
        undefined;
      ...
      if (status === undefined && !data) {
        console.warn(
          "onTest returned no status/data — check the onTest implementation",
        );
      }
    } catch (error) {
  ...
  const handleCurlParse = useCallback(
    (curlCommand: string) => {
      try {
        const parsed = parseCurlCommand(curlCommand);

        const { queryParams, pathParams } = parseUrlToParams(parsed.url);
  ...
  const applyExampleConfig = (example: "posts" | "todos" | "create") => {
    const preset =
      example === "posts"
        ? DUMMY_POSTS_TOOL_CONFIG
        : example === "todos"
          ? DUMMY_TODOS_TOOL_CONFIG
          : CREATE_POST_EXAMPLE;
        ...
        <div className="flex flex-col gap-4 h-full p-6 bg-white dark:bg-neutral-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700/40">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full sm:w-[90px] justify-between"
                >
                  {config.method}
            ...
            {[
              {
                value: "headers",
                label: "Headers",
                supportedTab: ["GET", "POST", "PUT", "PATCH", "DELETE"],
              },
              {
                value: "path",
                label: "Path",
            ...
            {["headers", "query", "path"].includes(activeTab) && (
              <div className="space-y-3">
                {activeTabMap?.map((activeTabMapItem: Parameter) => (
                  ...
                  >
                    <Input
                      ...
                      onChange={(e) =>
                        updateActiveTabMapItem(
                          activeTab as keyof typeof map,
                          activeTabMapItem.id,
                          "key",
                          e.target.value,
                        )
                ...
                <Button
                  ...
                  className="mt-4 h-10"
                  onClick={() =>
                    addActiveTabMapItem(activeTab as keyof typeof map)
                  }
                >
                  ...
                  {config.body?.map((bodyItem: Parameter) => (
                    <div key={bodyItem.id} className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        className="h-10 w-10 rounded-md border border-red-200/60 dark:border-red-900/30 text-red-600 dark:text-red-300 hover:bg-red-50/30 dark:hover:bg-red-900/25 transition-colors flex-shrink-0"
                        onClick={() =>
                          removeActiveTabMapItem("body", bodyItem.id)
                        }
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
          ...
          <div className="flex gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => applyExampleConfig("posts")}>
                  GET Posts
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => applyExampleConfig("todos")}>
                  GET Todos
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => applyExampleConfig("create")}>
                  POST Create
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
